...
class Order {
  ...
  addToOrderList(orderId, { title, price, pk_course_id, count }) {
    ...
    const candidate = this.orders[idx].orderList.findIndex(
      (course) => course.pk_course_id === pk_course_id
    );
    this.orders[idx].price = +this.orders[idx].price + price * count;
    if (candidate !== -1) {
      this.orders[idx].orderList[candidate].count =
        +this.orders[idx].orderList[candidate].count + count;
    } else {
      const course = {
        title,
        price,
  ...
  removeFromOrderList(orderId, courseId) {
    const idx = this.orders.findIndex(({ id }) => id === orderId);
    const idxCourse = this.orders[idx].orderList.findIndex(
      ({ pk_course_id }) => pk_course_id === courseId
    );
    this.orders[idx].price =
      +this.orders[idx].price -
      +this.orders[idx].orderList[idxCourse].price *
        +this.orders[idx].orderList[idxCourse].count;
    this.orders[idx].orderList = this.orders[idx].orderList.filter(
      ({ pk_course_id }) => pk_course_id !== courseId
    );
  }
